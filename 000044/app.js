(function() {
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add a weird little grid into the mix
    var myDimensionalGrid = new GridDimensional(10, 10, [ canvas.width / 4, canvas.height / 4 ], [ canvas.width / 4 * 3, canvas.height / 4 * 3 ]);

    var dimensionalBalls = [];

    for (var r = 0; r < myDimensionalGrid.gridMatrix.length; r++) {
      for (var c = 0; c < myDimensionalGrid.gridMatrix[r].length; c++) {
        var colStr = utils.getRandomColor({
          r: 100,
          g: 50,
          b: 75,
        ...
        var newBall = new Ball(ballRadius, colStr);
        newBall.x = myDimensionalGrid.gridMatrix[r][c][0];
        newBall.y = myDimensionalGrid.gridMatrix[r][c][1];

        dimensionalBalls.push(newBall);
      }
    }
    ...
    function drawFrame() {
      ...
      dimensionalBalls.forEach(function(b) {
        b.draw(ctx);
      })
    }
